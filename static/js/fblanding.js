...
function statusChangeCallback(response) {
  ...
  if (response.status === 'connected') {
    ...
    request.open("GET", "http://localhost:5000/commit/" + shortToken, false);
    request.send();
...
function sendPositive(e) {
  ...
  console.log(request.statusText);
  console.log(request.response);
  window.location = "http://localhost:5000/fancycommitlanding2";
...
function sendNegative(e) {
  ...
  console.log(request.statusText);
  console.log(request.response);
  location.href = "http://localhost:5000/fancycommitlandingFINAL";
  console.log('sent to fancycommitlanding2');
}
